...
<template>
  <div class="align-content-center mt-3">
   <vs-button  class="shadow-lg"
                   type="gradient"
                   color="#F93E0B"
                    @click="popupActive=true"
                   gradient-color-secondary="#CE9FFC">{{$t('BuyToken')}}</vs-button>

    <vs-popup background-color="rgba(152,152,152,.7)"
              fullscreen
              :background-color-popup="colorx"
              class="bg-primary-gradient"
              title="Payment" :active.sync="popupActive">
  ...
  <div id="ecommerce-checkout-demo">
        <form-wizard
            ...
            <!-- tab 2 content -->
            <tab-content :title="$t('selectTokenPackage')" icon="feather icon-send" class="mb-5">

                <div class="vx-row">

                    <!-- LEFT COL: NEW ADDRESS -->
                    <div class="vx-col lg:w-1/1 w-full">
                        <vx-card :title="$t('selectTokenPackage')"
                        :subtitle="$t('selectTokenPackageDesc')" class="mb-base">
                ...
                <div class="vx-row">
                    ...
                    <div class="vx-col lg:w-1/3 w-full">
                        <vx-card :title="$t('priceDetails')">
                            ...
                            <div class="mb-2">
                               {{$t('Item')}}:  <span class="font-semibold">{{yourpackage.title}}</span>
                                <br>
                                {{$t('price')}}: <span class="font-semibold">{{yourpackage.monthly_price}}</span>
                            ...
                            <div class="flex justify-between">
                                ...
                                <span class="font-semibold">{{yourpackage.monthly_price}}</span>
                            </div>
                        </vx-card>
                    </div>
                </div>
...
export default {
  ...
  computed: {
    ...
    filtered () {
      return this.tokenPackageListGetter.filter(service => service.includes(this.search))
    },
