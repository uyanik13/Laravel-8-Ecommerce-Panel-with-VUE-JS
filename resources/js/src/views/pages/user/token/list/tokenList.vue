      ...
      <vs-tabs position="left" color="danger">
        <vs-tab :label="$t('Account')"  icon-pack="feather" icon="icon-monitor">
          <div class="vx-row">
            ...
            <div class="vx-col w-full sm:w-1/1 md:w-1/1 lg:w-1/3 xl:w-1/3 mb-base">
              <statistics-card-line icon="ShoppingBagIcon" :statistic="Number(completedTokens)" :statisticTitle="$t('CompletedPayments')"  color="warning" type="area"></statistics-card-line>
            </div>
        ...
        <vs-tab :label="$t('OutgoingPayments')" icon-pack="feather" icon="icon-arrow-up">
          <div class="vx-col w-full lg:w-1/1 mb-base">
        ...
        <vs-tab :label="$t('IncomingPayments')" icon-pack="feather" icon="icon-arrow-down">
          <div class="vx-col w-full lg:w-1/1 mb-base">
        ...
        <vs-tab :label="$t('TokenOperations')" icon-pack="feather" icon="icon-dollar-sign">
          <div class="vx-row">
            <div class="vx-col w-full sm:w-1/2 lg:w-2/4 mb-base">
              <vx-card>
                <div class="my-8">
                  <h2 class="mb-2 text-4xl ">{{Number(activeUser.token ? activeUser.token : 0)}}</h2>
                  <p class="text-grey">{{$t('TotalTokens')}}</p>
                </div>
                <div class="flex justify-between flex-wrap">
                  <vs-button @click="setTokenRequest" v-show="!isAlreadyRequested" type="gradient" color="#F93E0B" gradient-color-secondary="#CE9FFC">{{$t('RequestPayment')}}</vs-button>
                </div>
              </vx-card>
            </div>
            <div class="vx-col w-full sm:w-1/2 lg:w-2/4 mb-base">
              <vx-card>
                ...
                <div class="my-6">
                  <feather-icon icon="ShoppingCartIcon" svg-classes="h-12 w-12" class="ml-4"></feather-icon>
...
export default {
  ...
  methods: {
    ...
    setTokenRequest () {
      this.$store.dispatch('token/addTokenRequest').then((response) => {
        this.showAlert(i18n.t('Success'), i18n.t('paymentRequestHasSent'), 'icon-success', 'success')
      }).catch(error => {
        this.showAlert(i18n.t('Error'), i18n.t('yourTokenIsUnderLimit'), 'icon-alert-circle', 'danger')
      })
    },
