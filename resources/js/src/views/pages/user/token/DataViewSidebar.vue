              ...
              <vs-tabs>
                <vs-tab :label="$t('textArea')">
                  <div class="mt-3">
                    ...
                    <div class="flex flex-wrap items-center mb-20 mt-20 h-full">
                      ...
                      <quill-editor  @change="onEditorChange($event)" :options="editorOption" :label="$t('postContent')" name="dataDescription" v-model="dataDescription"></quill-editor>
                ...
                <vs-tab :label="$t('ImageArea')">
                  <div class="vx-col w-full sm:w-10/12 lg:w-2/5 mb-base">
                    <vx-card>
                      <div slot="no-body">
                        <img :src="image" alt="Resim Yükle" class="responsive card-img-top">
                      </div>
                ...
                <vs-tab v-show="dataCategory === 5" :label="$t('VideoArea')">
                  <div class="vx-col w-full sm:w-10/12 lg:w-2/5 mb-base">
                      <vx-card >
                        <div class="flex justify-between flex-wrap">
                          <vs-input :label="$t('YouCanAlsoInputUrl')" v-model="video" class="mt-5 w-full" name="dataName" v-validate="'required'" />
                        </div>
                      </vx-card>
                   </div>
                </vs-tab>
  ...
  watch: {
    isSidebarActive (val) {
      ...
      if (!this.data.data) {
        ...
        const {  id, title, content, status, image, video, seo_title, seo_description} = JSON.parse(JSON.stringify(this.data.data))
