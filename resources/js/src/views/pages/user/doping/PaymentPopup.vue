...
<template>
 ...
 <vs-button class="mt-4 ml-8 shadow-lg" type="gradient" color="#E8603B" gradient-color-secondary="#E8603B" @click="popupActive=true">{{$t('GetStarted')}}</vs-button>
    <vs-popup background-color="rgba(152,152,152,.7)"

              :background-color-popup="colorx"
              class="bg-primary-gradient"
              :title="item.title" :active.sync="popupActive">
...
<div id="ecommerce-checkout-demo">
        <form-wizard
            ...
            <!-- tab 2 content -->
            <tab-content :title="$t('selectService')" icon="feather icon-send" class="mb-5">

                <div class="vx-row">

                    <!-- LEFT COL: NEW ADDRESS -->
                    <div class="vx-col lg:w-1/1 w-full">
                        <vx-card :title="$t('pleaseSelectAService')"
                        :subtitle="$t('pleaseSelectAServiceDesc')" class="mb-base">
                  ...
                  <span v-show="errors.has('selectService.yourService')"
                   class="text-danger">{{ errors.first('selectService.yourService') ? $t('pleaseSelectAService'): '' }}</span>

                                    </div>


                                </div>
                                      <vs-button class="mt-6 ml-auto flex" @click.prevent="submitNewAddressForm">{{$t('goToPayment')}}</vs-button>
                            </form>
                        </vx-card>
                    </div>
            ...
            <tab-content :title="$t('Payment')" icon="feather icon-credit-card" class="mb-5">
                ...
                <div class="vx-row">
                    ...
                    <div class="vx-col lg:w-1/1 w-full">
                        <vx-card :title="$t('Payment')"  :subtitle="$t('paymentDesc')" class="mb-base">

                            <div class="mt-3">
                          <span class="mr-3">{{$t('forThisActionCutting')}} {{!PriceAnnual ? item.monthly_price : item.yearly_price }} {{$t('Token')}}</span>
              <vs-button class="mt-4"
               @click="makePayment(item,PriceAnnual)"
               :disabled="false">{{$t('PayNow')}}</vs-button>
                                <vs-divider />
...
export default {
  ...
  computed: {
    ...
    filtered () {
      return this.serviceListGetter.filter(service => service.includes(this.search))
    },
  ...
  methods: {
    ...
    makePayment (item, PriceAnnual) {
      ...
      this.$store.dispatch('subscription/addItem', obj)
        .then((response) => {
          Swal.fire({
            type: 'warning',
