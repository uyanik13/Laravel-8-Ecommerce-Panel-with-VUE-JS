        ...
        <div class="vx-row match-height">
          <div class="vx-col w-full md:w-11/12 mb-base">
            <vx-card>
              ...
              <!--  USERS -->
              <vs-select v-model="user_id" :label="$t('Users')" name="dataUserId" class="mt-5 w-full">
                <vs-select-item :key="user.id" :value="user.id" :text="user.name" v-for="user in userList" />
                <span class="text-danger text-sm" v-show="errors.has('user_id')">{{ errors.first('user_id') }}</span>
              </vs-select>

              <!--  PACKAGES -->
              <vs-select v-model="package_id" :label="$t('Packages')" name="dataUserId" class="mt-5 w-full">
                <vs-select-item :key="packageData.id" :value="packageData.id" :text="packageData.title" v-for="packageData in packageList" />
                <span class="text-danger text-sm" v-show="errors.has('package_id')">{{ errors.first('package_id') }}</span>
              </vs-select>

                  <!-- TITLE -->
              <vs-input :label="$t('Title')" v-model="title" class="mt-5 w-full" name="title" v-validate="'required'" />
              ...
              <!-- PRICE -->
              <vs-input :label="$t('Price')" v-model="amount" class="mt-5 w-full" v-validate="'required'" />
              ...
              <datepicker placeholder="Select Date" v-model="ends_at"></datepicker>
  ...
  watch: {
    isSidebarActive (val) {
      ...
      if (!this.data.data) {
        ...
        const {id, user_id, package_id, title, start_at, ends_at, amount} = JSON.parse(JSON.stringify(this.data.data))
      ...
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          if (this.dataId !== null && this.dataId >= 0) {
            this.$store.dispatch('subscription/updateItem', obj).then((response) => {
            ...
            delete obj.id
            this.$store.dispatch('subscription/addItem', obj).then((response) => {
