          ...
          <vs-tabs>
                ...
                :label="$t(locale)"
                    ...
                    icon="flag">
                            ...
                            <!--  TITLE -->
                     <vs-divider background="primary" color="#ade6d4">{{$t('postName')}}</vs-divider>
                    ...
                    <vs-divider background="primary" color="#ade6d4">{{$t('postContent')}}</vs-divider>
                         <!--  CONTENT -->
                    <div class="vx-row mb-10">
                    ...
                    <vs-divider background="primary" color="#ade6d4">{{$t('others')}}</vs-divider>
                    <vs-tabs>

                        <!-- PRICE -->
                        <vs-tab :label="$t('price')" icon-pack="material-icons" icon="attach_money">
                             <vs-input :label="$t('Price')"  v-model="price" class="mt-1 w-full" />
                        </vs-tab>

                        <!-- DISCOUNTED PRICE -->
                       <vs-tab :label="$t('discounted_price')" icon-pack="material-icons" icon="money_off">
                          <vs-input :label="$t('discounted_price')"  v-model="discounted_price" class="mt-1 w-full" />
                       </vs-tab>

                        <!-- QUANTITY -->
                       <vs-tab :label="$t('quantity')" icon-pack="material-icons" icon="pie_chart">
                        <vs-input-number  :label="$t('Quantity')" class="mt-10 w-full" v-model="quantity"/>
                       </vs-tab>

                        <!-- TAGS -->
                       <vs-tab :label="$t('tags')" icon-pack="material-icons" icon="local_offer">
                         <span>{{$t('tags') + '-' + locale.toUpperCase()}}</span>
                        <v-select taggable push-tags multiple :closeOnSelect="false" v-model="tags[locale]"  :dir="$vs.rtl ? 'rtl' : 'ltr'" /><br>
                       </vs-tab>

                        <!-- OPTIONS -->
                       <vs-tab :label="$t('options')" icon-pack="material-icons" icon="done_outline">
                         <div class="vx-row my-10  bg-black-400" v-for="(option,index)  in options[locale].features" :key="index" >
                            ...
                            <div class="vx-col sm:w-1/12 mt-12">
                                 <feather-icon @click="removeThis(index,locale,'features')" icon="XCircleIcon" svgClasses="w-10 h-10 text-red"></feather-icon>
                            </div>
                            <div class="vx-col sm:w-1/12 mt-12">
                                 <feather-icon @click="addOptions(locale,'features')" icon="PlusCircleIcon" svgClasses="w-10 h-10 text-red"></feather-icon>
                            </div>
                         </div>
                        ...
                        <!-- CATEGORIES -->
                       <vs-tab :label="$t('categories')" icon-pack="material-icons" icon="category">
                        ...
                        <!-- STATUS -->
                        <vs-tab :label="$t('status')" icon-pack="material-icons" icon="pie_chart">
              ...
              <vs-divider background="primary" color="#ade6d4">{{$t('seoSettings')}}</vs-divider>

              <vs-tabs>
                     <!-- SEO TITLE -->
                    <vs-tab :label="$t('seoTitle')" icon-pack="material-icons" icon="toc">
                      ...
                      <!-- SEO DESCRIPTION -->
                    <vs-tab :label="$t('seoDescription')" icon-pack="material-icons" icon="description">
            ...
            <vs-divider background="primary" color="#ade6d4">{{$t('imageSettings')}}</vs-divider>


                 <vs-tabs class="my-10">
                    <vs-tab :label="$t('thumbnailUpload')"  icon-pack="material-icons" icon="add_a_photo">
                            ...
                            <div class="vx-col sm:w-3/12 mt-5 mb-5" >
                                <vx-card class="overlay-card overflow-hidden">
                                    ...
                                    <vs-button
                                            ...
                                            <feather-icon
                                            icon="ImageIcon" svgClasses="w-25 h-25 text-white">
                                            </feather-icon>
                                            {{$t('selectImage')}}
                                    </vs-button>
                                    <input type="file"  class="hidden" :ref="'thumbnail'+locale" @change="imageUpload($event,locale,'thumbnail')" accept="image/*">
                                    <img :src="thumbnail[locale]" :alt="$t('thumbnailUpload')" height="200px" width="200px" class="card-img-top m-5" />
                                    </template>
                                </vx-card>
                            </div>
                        </div>
                    </vs-tab>
                    <vs-tab :label="$t('galleryUpload')" icon-pack="material-icons" icon="collections">

                    <div class="vx-row bg-grey">
                        <div class="vx-col sm:w-3/12 mt-5 mb-5" v-for="(image,index)  in images" :key="index">
                          <vx-card class="overlay-card overflow-hidden">
                          <img v-if="!image.slug"  src="https://via.placeholder.com/800.png"  height="200px"  width="200px" class="card-img-top" />
                          <img v-else :src="image.slug"   height="200px"  width="200px" class="card-img-top" />
                           <vs-input :label="$t('altText')"  v-model="image.alt_text" class="mt-5 w-full"  />
                          <input type="file"  class="hidden" :ref="'images'+index" @change="galeryUpload($event,index,'images')" accept="image/*">
                            ...
                            <vs-button
                                      ...
                                      <feather-icon
                                      icon="ImageIcon" svgClasses="w-25 h-25 text-red">
                                      </feather-icon>
                            </vs-button>
                            <vs-button
                                      class="my-3"
                                      ...
                                      <feather-icon
                                      icon="XCircleIcon" svgClasses="w-25 h-25 text-red">
                                      </feather-icon>
                            </vs-button>
                         ...
                         <div class="vx-col sm:w-3/12 mt-12">
                            <vs-button
                                ...
                                <feather-icon  icon="PlusCircleIcon" svgClasses="w-32 h-32 text-red"></feather-icon>
                             </vs-button>
    ...
    export default {
        ...
        computed: {
            ...
            isFormValid () {
             ...
             filtered () {
                return this.getChildCategories().filter(category => category.includes(this.search))
            },
        ...
        methods: {
           ...
           openFileInput (type) {
            ...
            let {  id, title,content,seo_title,category_id,price,discounted_price,options,quantity,thumbnail,tags,galleries,status,seo_description} = JSON.parse(JSON.stringify(this.data.data))
                    //console.log('active:',)
                    this.id = id
                    this.title = title
            ...
            initValues () {
                ...
                this.thumbnail = {'en': 'https://via.placeholder.com/800.png','tr': 'https://via.placeholder.com/800.png','de': 'https://via.placeholder.com/800.png'}
                this.tags = {'en': [],'tr': [],'de': []}
                this.price = 100
            ...
            filteringGallery (data) {
                return data.flatMap(({ alt_text, slug }) => Object.assign({ alt_text, slug }))
            },
