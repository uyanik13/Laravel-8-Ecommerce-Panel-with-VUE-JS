<template>
  <div class="vx-col w-full md:w-12/12 mb-base pr-20 pl-5">
   <global-file-manager :showPopup="showPopup" @embed-media-from-popup="embedMediaFromPopup" />
           ...
           <vx-card v-for="(option,index)  in Testimonials" :key="index" class="mb-5 ml-2 mr-5 mt-5" >
                  <template slot="actions">
                         <feather-icon @click="removeThis(index,'Testimonials')" icon="XCircleIcon" svgClasses="w-10 h-10 text-red"></feather-icon>
                    </template>
                    ...
                    <div class="vx-row">
                        ...
                        <div class="vx-col sm:w-1/2 lg:w-1/1 mb-2">
                            <vs-textarea :label="'Comment'"  v-model="option.comment" class="mt-5 w-full"  />
                        </div>
                        ...
                        <div>
                            <input type="file"  class="hidden" :ref="'Testimonials'+index" @change="sliderUpload($event,index,'Testimonials')" accept="image/*">
                            <vs-button
                                            class="ml-5 my-5 center"
                                            color="#F93E0B"
                                            @click="embedImage('Testimonials',index)">
                                            <feather-icon
                    ...
                    <vs-button
                       ...
                       @click="addOptions('Testimonials')">{{$t('AddNew')}}
                  </vs-button>


                <div class="flex flex-wrap items-center justify-end">
                    <vs-button class="ml-auto mt-2" @click="SaveData('Testimonials')">{{$t('save')}}</vs-button>
                </div>
    ...
    this.$store.dispatch('custom/fetchItems')
      .then((response) => {
        response.data.forEach(element => {
          if (element.type === 'Testimonials' || element.type === 'fourBox') {
