...
<template>
  <div id="data-list-list-view" class="data-list-container">
    ...
    <vs-table ref="table"  pagination :max-items="itemsPerPage" search :data="blogPosts">
      ...
      <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
        ...
        <div class="flex flex-wrap-reverse items-center data-list-btn-container">
          ...
          <div class="btn-add-new p-3 mb-4 mr-4 rounded-lg cursor-pointer flex items-center justify-center text-lg font-medium text-base text-primary border border-solid border-primary" @click="addNewData">
              ...
              <span class="ml-2 text-base text-primary">{{$t('addCategory')}}</span>
          </div>
        </div>
        ...
        <template slot-scope="{data}">
          <tbody>
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td>
                <p class="product-name font-medium truncate">{{ tr.slug[locale] }}</p>
              </vs-td>
              <vs-td>
                <p class="product-name font-medium truncate">{{ tr.seo_title[locale] }}</p>
              </vs-td>
              <vs-td>
                <p class="product-name font-medium truncate">{{ tr.seo_description[locale] }}</p>
              </vs-td>


              <vs-td class="whitespace-no-wrap">
                <feather-icon icon="EditIcon"  svgClasses="w-7 h-7 hover:text-primary stroke-current" @click="editData(tr)" />
                <feather-icon icon="TrashIcon" v-show="tr.id !== 1 && tr.id !== 2 && tr.id !== 3 && tr.id !== 4"  svgClasses="w-7 h-7 hover:text-danger stroke-current" class="ml-2" @click.stop="deleteData(tr.id)" />
              </vs-td>
  ...
  methods: {
    ...
    deleteData(id) {
      Swal.fire({
        ...
        if (result.value) {
          console.log(result)
          this.$store.dispatch("categoryModule/removeItem", id)
            .then((response) => {
              this.$vs.notify({
                title: 'Başarılı',
