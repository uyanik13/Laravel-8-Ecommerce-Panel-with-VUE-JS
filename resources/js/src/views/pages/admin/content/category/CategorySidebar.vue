            ...
            <vx-card>
              <vs-tabs>
                <vs-tab
                    ...
                    <!-- NAME -->
                    <vs-input :label="$t('title') + '-' + locale.toUpperCase()" v-model="title[locale]" class="mt-5 w-full" name="title" v-validate="'required'" />
...
import i18n from '@/i18n/i18n'
  ...
  export default {
    ...
    computed: {
     ...
     filtered () {
      return this.categories.filter(category => category.includes(this.search))
    },
    categories () {
    ...
    methods: {
      watchDataWhenSidebarActive (val){
        ...
        if(this.data.newData) {
          ...
          let {  id, title,seo_title,parent_id,seo_description} = JSON.parse(JSON.stringify(this.data.data))
        ...
        this.$vs.loading()
          if (result) {
            ...
            if(this.id !== null && this.id >= 0) {
              this.$store.dispatch("categoryModule/updateItem", obj).then((response) => {
              ...
              delete obj.id
              this.$store.dispatch("categoryModule/addItem", obj).then((response) => {
                this.$vs.loading.close()
                    this.$vs.notify({
                        title: i18n.t('success'),
