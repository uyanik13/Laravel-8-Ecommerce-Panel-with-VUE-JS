          ...
          <vs-tabs>
                ...
                :label="$t(locale)"
                    ...
                    icon="flag">
                            ...
                            <!--  TITLE -->
                     <vs-divider background="primary" color="#ade6d4">{{$t('packageTitle')}}</vs-divider>
                    <div class="vx-row mb-10">
                            <div class="vx-col sm:w-1/1 w-full">
                                <vs-input :label="$t('packageTitle') + '-' + locale.toUpperCase()" v-model="title[locale]" class="mt-5 w-full" name="title" v-validate="'required'" />
                                 <span class="text-danger text-sm" v-show="errors.has('title')">{{ errors.first('title') ? $t('titleRequired')  : ''}}</span>
                            </div>

                     </div>

                    <vs-divider background="primary" color="#ade6d4">{{$t('packageSecondTitle')}}</vs-divider>
                         <!--  CONTENT -->
                        <div class="vx-row mb-10">
                            <div class="vx-col sm:w-1/1 w-full">
                                <vs-input :label="$t('packageSecondTitle') + '-' + locale.toUpperCase()" v-model="second_title[locale]" class="mt-5 w-full" name="title" v-validate="'required'" />
                                 <span class="text-danger text-sm" v-show="errors.has('second_title')">{{ errors.first('second_title') ? $t('second_title_required')  : ''}}</span>
                            </div>

                     </div>


                 <vs-divider background="primary" color="#ade6d4">{{$t('price')}}</vs-divider>
                    <vs-tabs>
                        <vs-tab :label="$t('MonthlyPrice')">
                        <vs-input :label="$t('MonthlyPrice')" v-model="monthly_price" class="mt-5 w-full" name="monthly_price"  v-validate="'required'" />
                        <span class="text-danger text-sm" v-show="errors.has('monthly_price')">{{ errors.first('monthly_price') ? $t('MonthlyPriceRequired'): ''}}</span>
                        </vs-tab>
                        <vs-tab :label="$t('YearlyPrice')">
                        <vs-input :label="$t('YearlyPrice')" v-model="yearly_price" class="mt-5 w-full" name="yearly_price"  v-validate="'required'" />
                        <span class="text-danger text-sm" v-show="errors.has('yearly_price')">{{ errors.first('yearly_price') ? $t('YearlyPriceRequired'): ''}}</span>
                        </vs-tab>
                </vs-tabs>
                    ...
                    <vs-tabs>
                       ...
                       <vs-tab :label="$t('options')" icon-pack="material-icons" icon="done_outline">
                         <div class="vx-row my-4  bg-black-400" v-for="(option,index)  in description[locale]" :key="index" >

                            <div class="vx-col sm:w-5/12">
                                <vs-input :label="$t('Value')"  v-model="option.value" class="mb-1 w-full" />
                            </div>
                            <div class="vx-col sm:w-1/12 mt-12">
                                 <feather-icon @click="removeThis(index,locale)" icon="XCircleIcon" svgClasses="w-10 h-10 text-red"></feather-icon>
                            </div>
                            <div class="vx-col sm:w-1/12 mt-12">
                                 <feather-icon @click="addOptions(locale)" icon="PlusCircleIcon" svgClasses="w-10 h-10 text-red"></feather-icon>
                            </div>
                         </div>
                       </vs-tab>
                </vs-tabs>
 ...
 data () {
  ...
  methods: {
     watchDataWhenSidebarActive (val){
            if(!val) return
                if(this.data.newData) {
                 ...
                 let {  id, title,second_title,description,thumbnail,monthly_price,yearly_price,type,popular} = JSON.parse(JSON.stringify(this.data.data))
                    //console.log('active:',)
                    this.id = id
                    this.title = title
    ...
    initValues () {
                ...
                this.second_title = {'en': 'en - '+i18n.t('second_title'),'tr': 'tr - '+i18n.t('second_title'),'de': 'de - '+i18n.t('second_title')}
      ...
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          if (this.id !== null && this.id >= 0) {
            this.$store.dispatch('package/updateItem', obj).then((response) => {
              this.showAlert(i18n.t('Success'), i18n.t('SuccessfullyUpdated'), 'icon-success', 'success')
            }).catch(error => {
              this.showAlert(i18n.t('Error'), i18n.t('CouldntUpdated'), 'icon-alert-circle', 'danger')
            })
          } else {
            delete obj.id
            this.$store.dispatch('package/addItem', obj).then((response) => {
